...
import { calculateSpan, timeToSlot, DAY_NAME_MAP } from '@/shared/lib/timetable-utils';
import { TimetableHeader } from './components/TimetableHeader';
import { TimetableGridContent } from './components/TimetableGridContent';
...
export function TimetableDisplay({ 
  ...
  const [, setHoveredIndex] = React.useState<string | null>(null);
  ...
  const timetableData: { [day: string]: { [slot: number]: Array<IndexData & { isStart: boolean }> } } = {};
  ...
  indexes.forEach(index => {
    ...
    for (let i = 0; i < span; i++) {
      ...
      const exists = timetableData[normalizedDay][slotNum].some(
        existing => 
          existing.indexNumber === index.indexNumber && 
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-background/90 backdrop-blur-sm px-6 py-4 rounded-lg border shadow-sm">
            <div className="text-center space-y-2">
              ...
              <p className="text-xs text-muted-foreground/70">Select indexes to preview them here</p>
            </div>
          </div>
        </div>
      )}
