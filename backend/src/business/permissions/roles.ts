...
export const ALLOWED_ROLES: UserRole[] = ['superadmin', 'admin', 'pro', 'plus', 'user', 'default'];
...
export function hasRequiredRole(role: string | null | undefined, required: UserRole): boolean {
...
export function isAllowedRole(role: string): boolean {
  ...
  if (cleaned in ROLE_ALIASES) return true;
  return ALLOWED_ROLES.includes(cleaned as UserRole);
}
